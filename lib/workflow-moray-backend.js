  ...
  self.client.put('wf_workflows', uuid, workflow, function (err, meta) {
    ...
    if (err) {
      if (err.code === 'Invalid Argument') {
        return callback(new wf.BackendInvalidArgumentError(err.message));
      } else {
...
WorkflowMorayBackend.prototype.getWorkflow = function (uuid, callback) {
  ...
  self.client.get('wf_workflows', uuid, function (err, meta, workflow) {
    if (err) {
      ...
      // We're saving the uuid as key, need to add it back to the workflow:
      workflow.uuid = uuid;
      return callback(null, workflow);
    }
  });
...
WorkflowMorayBackend.prototype.createJob = function (job, callback) {
  ...
  self.client.put('wf_jobs', uuid, job, function (err, meta) {
    job.uuid = uuid;
    if (err) {
      // TODO: Review for InvalidArgument
      return callback(new wf.BackendInternalError(err.message));
    } else {
  ...
  return self.client.search('wf_jobs', filter, function (err, data) {
    ...
    if (keys.length === 0) {
      ...
      return async.forEach(keys, function (k, cb) {
        if (data[k].params &&
            self._areParamsEqual(job.params, JSON.parse(data[k].params))) {
...
WorkflowMorayBackend.prototype.nextJob = function (index, callback) {
  var self = this,
      ...
      // TODO: Sorting is pending. We want 'ORDER BY created_at ASC'
    ...
    if (err) {
      ...
      return self._decodeJob(job, function (j) {
        ...
        if (theJob.execution !== 'queued') {
          ...
          return self.client.put('wf_jobs', uuid, theJob, {
            etag: etag
          }, function (err, meta) {
            if (err) {
...
WorkflowMorayBackend.prototype.finishJob = function (job, callback) {
  ...
  self.getJob(job.uuid, function (err, aJob) {
    if (err) {
      return callback(err);
    } else {
      return self._decodeJob(aJob, function (j) {
        if (j.execution !== 'running') {
          ...
          job.runner_id = null;
          return self.client.put('wf_jobs', uuid, job, function (err, meta) {
            job.uuid = uuid;
            if (err) {
              if (err.code === 'Invalid Argument') {
                return callback(
...
// - callback - f(err, jobs)
WorkflowMorayBackend.prototype.getJobs = function (execution, callback) {
  var self = this,
...
WorkflowMorayBackend.prototype.addInfo = function (uuid, info, callback) {
  ...
  self.client.get('wf_jobs', uuid, function (err, meta, job) {
    if (err) {
      ...
      return self._decodeJob(job, function (j) {
        theJob = j;
        theJob.info = theJob.info || [];
        theJob.info.push(info);
        job.info = JSON.stringify(theJob.info);
        return self.client.put('wf_jobs', uuid, job, function (err, meta) {
          if (err) {
            if (err.code === 'Invalid Argument') {
              return callback(new wf.BackendInvalidArgumentError(err.message));
...
WorkflowMorayBackend.prototype._decodeJob = function (job, callback) {
  ...
  if (job.info && typeof (job.info) === 'string') {
