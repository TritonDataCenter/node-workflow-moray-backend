...
WorkflowMorayBackend.prototype.init = function (callback) {
    ...
    self.client.on('error', function (err) {
        self.log.error({err: err}, 'Moray client error');
        self.emit('error', err);
    });

    self.client.on('connectAttempt', function (moray, number, delay) {
        self.log.info({
            attempt: number,
            delay: delay,
...
WorkflowMorayBackend.prototype.validateJobTarget = function (job, callback) {
    ...
    var req = self.client.findObjects('wf_jobs', filter);
    var is_dup = false;
    ...
    var onRecord = function (obj) {
        ...
        try {
            ...
            self.log.error(e, 'findObject exception');
            req.emit('error', e);
        }
    };
...
WorkflowMorayBackend.prototype._putJob = function (uuid, job, opts, callback) {
    ...
    try {
        ...
        self.log.error(e, 'Exception trying to put Job to Moray');
