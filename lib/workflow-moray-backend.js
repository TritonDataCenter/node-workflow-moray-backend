...
var WorkflowMorayBackend = module.exports = function (config) {
    ...
    function _createWfLockedTargetsBucket(_, next) {
        return _bucketExists('wf_locked_targets', function (exists) {
            if (!exists) {
                return client.putBucket('wf_locked_targets', {
                    index: {
                        target: {
                            type: 'string'
                        }
    ...
    function _decodeJob(job, chain, callback) {
        ...
        Object.keys(job).forEach(function (p) {
            ...
            case 'onerror':
                if (chain === true) {
                    if (typeof (job[p]) === 'string') {
                        job[p] = JSON.parse(job[p]);
                    }
                } else {
                    delete job[p];
                }
            ...
            case 'created_at':
            case 'exec_after':
                if (typeof (job[p]) === 'number') {
                    job[p] = new Date(job.created_at).toISOString();
                }
                break;
            default:
                break;
                ...
                return _decodeJob(aJob, true, function (j) {
                    if (j.execution !== 'running') {
                        ...
                        return _putJob(uuid, job, {}, function (err, job) {
                            if (typeof (job.locks) !== 'undefined') {
                                client.delObject('wf_locked_targets', job.uuid,
                                    function (err1) {
                                        if (err1) {
                                            return callback(err1, job);
                                        }
    ...
    function getJobs(params, callback) {
        ...
        if (typeof (vm_uuid) !== 'undefined') {
            pieces.push('(target=*' + vm_uuid + '*)');
        }
