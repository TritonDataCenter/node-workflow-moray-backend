...
const JOB_STATUS_CHANGED_NOTIFY_CHANNEL = 'workflow_job_status_changed';
...
var WorkflowMorayBackend = module.exports = function (config) {
    ...
    // Internal method used to send a moray workflow status change notification.
    function _notifyJobStatusChange(job, fromStatus, callback) {
        var lastResult = Array.isArray(job.chain_results) &&
            job.chain_results.slice(-1)[0];
        // Note that we only send through a small amount of the workflow job,
        // as the notify payload is limited in size (8000 bytes), just send the
        // most important details.
        var event = {
            prevExecution: fromStatus,
        ...
        client.notify(JOB_STATUS_CHANGED_NOTIFY_CHANNEL, payload, callback);
    }
                ...
                return _decodeJob(aJob, true, function (j) {
                    if (from_status !== null && j.execution !== from_status) {
                        ...
                        return _putJob(uuid, job, function (err, theJob) {
                            ...
                            _notifyJobStatusChange(theJob, from_status,
                                    function (err) {
                                // Log any error, it's not fatal if we could
                                // not send the notification.
                                if (err) {
                                    log.warn('Unable to notify job status ' +
                                        'change: %s', err);
                                }
                                callback(null, theJob);
                            });
                        });
                    }
                });
    ...
    function finishJob(job, callback) {
        ...
        return _processJob(job, 'finishJob', null, to_status,
            function (err, theJob) {
                ...
                if (config.metricsManager) {
                    config.metricsManager.collectMetrics('jobMetrics', theJob);
                }
